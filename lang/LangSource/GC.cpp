...
{
	// obtain size info
	int32 alignedSize = (int)((inNumBytes + kAlignMask) & ~kAlignMask); // 16 byte align
	...
	int32 allocSize = (int)(sizeof(PyrObjectHdr) + (sizeof(PyrSlot) << sizeclass));
...
void PyrGC::ScanStack()
{
	...
	int32 size = obj->size = (int)(g->sp - slot + 1);
