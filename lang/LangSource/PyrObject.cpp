...
static void fillClassRow(PyrClass *classobj, PyrMethod** bigTable, boost::atomic<size_t> * rCount)
{
	...
	if (IsObj(&classobj->methods)) {
		...
		for (int i=0; i<methods->size; ++i) {
			PyrMethod* method = slotRawMethod(&methods->slots[i]);
			int selectorIndex = (int)(slotRawSymbol(&method->name)->u.index);
	...
	*rCount += count;
	...
	if (IsObj(&classobj->subclasses)) {
		...
		if (numSubclasses > 4*cpuCount) {
			int subclassesPerThread = (int)(numSubclasses / cpuCount);
			for (int i = 0; i != helperThreadCount; ++i)
				compileThreadPool.schedule(boost::bind(&fillClassRowSubClasses, subclasses,
													   ...
													   bigTable, rCount));
			fillClassRowSubClasses(subclasses, subclassesPerThread * helperThreadCount, (int)numSubclasses, bigTable, rCount);
		}
		else
			fillClassRowSubClasses(subclasses, 0, (int)numSubclasses, bigTable, rCount);
	}
}
...
int calcHash(PyrSlot *a)
{
	...
	case tagSym : hash = (int)slotRawSymbol(a)->hash; break;
