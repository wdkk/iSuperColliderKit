...
int doSpecialUnaryArithMsg(VMGlobals *g, int numArgsPushed)
{
	PyrSlot *a;
	PyrSymbol *msg;
	int opcode = (int)g->primitiveIndex;    // kengo:
			...
			switch (GetTag(b)) {
				case tagInt :
					switch (opcode) {
						...
						case opIDiv :		SetInt(a, (int)floor(slotRawFloat(a) / slotRawInt(b))); break;  // kengo:long->int
				...
				default : // double
					switch (opcode) {
						...
						case opIDiv :		SetInt(a, (int)floor(slotRawFloat(a) / slotRawFloat(b))); break;    // kengo:long->int
