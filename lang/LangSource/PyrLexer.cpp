...
#include "SC_TextUtils.h"   // kengo: hpp -> h
...
{
	uint32 len = (int)strlen(gCompileDir);
...
static bool getFileText(char* filename, char **text, int *length)
{
	...
	size_t size = (int)fread(ltext, 1, llength, file);
	if (size != llength) {
...
start:
	...
	if (c == 0)   { r = 0; goto leave; }
		...
		if (!emptyls(&brackets)) {
			if ((d = (int)popls(&brackets)) != OPENSQUAR) {
				fatal();
		...
		if (!emptyls(&brackets)) {
			if ((d = (int)popls(&brackets)) != OPENPAREN) {
				fatal();
				post("opening bracket was a '%c', but found a '%c'\n",d,c);
				goto error2;
			}
		...
		if (!emptyls(&brackets)) {
			if ((d = (int)popls(&brackets)) != OPENCURLY) {
				fatal();
				post("opening bracket was a '%c', but found a '%c'\n",d,c);
				goto error2;
			}
			lastClosedFuncCharNo = (int)popls(&closedFuncCharNo);
		} else {
			fatal();
			post("unmatched '%c'\n",c);
			goto error2;
