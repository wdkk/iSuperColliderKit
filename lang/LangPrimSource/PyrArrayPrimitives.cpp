...
int basicMaxSize(struct VMGlobals *g, int numArgsPushed)
{
	...
	obj = slotRawObject(a);
	maxsize = (int)MAXINDEXSIZE(obj);   // kengo:
	SetInt(a, maxsize);
	return errNone;
}
...
int prArrayAdd(struct VMGlobals *g, int numArgsPushed)
{
	...
	maxelems = (int)MAXINDEXSIZE(array);    // kengo:
	if (array->size >= maxelems || array->IsImmutable()) {
		numbytes = (int)(sizeof(PyrSlot) << (array->obj_sizeclass + 1));    // kengo:
...
int prArrayInsert(struct VMGlobals *g, int numArgsPushed)
{
	...
	//const int tag = gFormatElemTag[format];       // kengo:
	...
	const int maxelems = (int)MAXINDEXSIZE(array);  // kengo:
	if (size+1 > maxelems || array->IsImmutable()) {
		oldarray = array;

        int numbytes = (int)(sizeof(PyrSlot) << (array->obj_sizeclass + 1));    // kengo:
		array = g->gc->New(numbytes, 0, format, true);
