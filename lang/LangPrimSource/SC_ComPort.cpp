...
void* SC_UdpInPort::Run()
{
	...
	while (true) {
		...
		int size = (int)recvfrom(mSocket, buf, kTextBufSize , 0,
...
void* SC_UdpCustomInPort::Run()
{
	...
	while (mRunning.load(boost::memory_order_consume)) {
		...
		if ((n > 0) && FD_ISSET(fd, &rfds)) {
			...
			packet->mReplyAddr.mSockAddrLen = sizeof(sockaddr_in);
			int size = (int)recvfrom(mSocket, buf, kTextBufSize , 0,
								(struct sockaddr *) &packet->mReplyAddr.mSockAddr,
                                     (socklen_t*)&packet->mReplyAddr.mSockAddrLen);
...
#else
    int numbytes = (int)recv(socket, msg, len - total, 0);
#endif
		if (numbytes <= 0) return (int)total;
		total += numbytes;
...
#else
		int numbytes = (int)recvfrom(socket, msg, len - total, 0, fromaddr, &addrlen2);
#endif
		if (numbytes < 0) return (int)total;
		total += numbytes;
		msg = (void*)((char*)msg + numbytes);
...
#else
		int numbytes = (int)sendto(socket, msg, len - total, 0, toaddr, addrlen);
#endif
...
#else
		int numbytes = (int)send(socket, msg, len - total, flags);
#endif
		if (numbytes < 0) return (int)total;
		total += numbytes;
		msg = (void*)((char*)msg + numbytes);
