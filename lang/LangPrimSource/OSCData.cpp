...
//const int ivxNetAddr_Hostname = 2;        // kengo:
...
int prNetAddr_SendMsg(VMGlobals *g, int numArgsPushed)
{
	...
	return netAddrSend(slotRawObject(netAddrSlot), (int)packet.size() /*kengo:*/, (char*)packet.buf);
}
...
int prNetAddr_BundleSize(VMGlobals *g, int numArgsPushed)
{
	...
	SetInt(args, (int)packet.size() /*kengo:*/);
	return errNone;
}
...
int prArray_OSCBytes(VMGlobals *g, int numArgsPushed)
{
	...
	int size = (int)packet.size();  // kengo:
...
static PyrInt8Array* MsgToInt8Array ( sc_msg_iter msg )
{
	int size = (int)msg.getbsize();     // kengo:
	VMGlobals *g = gMainVMGlobals ;
	PyrInt8Array *obj = newPyrInt8Array ( g->gc , size , 0 , true ) ;
	obj->size = size ;
...
{
	char *cmdName = inData;
	int cmdNameLen = (int)OSCstrlen(cmdName);   // kengo:
...
void PerformOSCBundle(int inSize, char* inData, PyrObject *replyObj, int inPortNum)
{
	...
	int numMsgs = (int)(g->sp - stackBase); // kengo:
...
int prBootInProcessServer(VMGlobals *g, int numArgsPushed)
{
	...
	if (!gInternalSynthServer.mWorld) {
		...
		static char /*mInputStreamsEnabled[512], mOutputStreamsEnabled[512],*/ mDeviceName[512];    // kengo:
		int err;
