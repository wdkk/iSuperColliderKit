...
int prAsArchive(struct VMGlobals *g, int numArgsPushed)
{
	...
	int err = (int)arch.prepareToWriteArchive(a);
	if (err) return err;
...
int prUnarchive(struct VMGlobals *g, int numArgsPushed)
{
	...
	int err = (int)arch.readArchive(a);
	return err;
}
...
int prWriteArchive(struct VMGlobals *g, int numArgsPushed)
{
	...
	int err = errNone;
	if (file) {
		err = (int)(arch.prepareToWriteArchive(a));
		if (!err) {
			arch.setStream(file);
			err = (int)arch.writeArchive();
		}
