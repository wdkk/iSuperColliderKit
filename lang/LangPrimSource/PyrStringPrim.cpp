...
static int prString_FindRegexp(struct VMGlobals *g, int numArgsPushed)
{
	...
	try {
		...
		while (start <= end && regex_search(start, end, what, pattern, flags))
		{
			for (int i = 0; i < what.size(); ++i )
			{
				...
				if (what[i].matched) {
					match.pos = (int)(what[i].first - stringBegin);
					match.len = (int)(what[i].second - what[i].first);
				} else {
					match.pos = 0;
					match.len = 0;
				}
	...
	int match_count = (int)matches.size();
...
int prString_Find(struct VMGlobals *g, int numArgsPushed)
{
	...
	if (cmp == 0) {
		SetInt(a, (int)(achar - slotRawString(a)->s));
	} else {
		SetNil(a);
	}
	return errNone;
...
static void yaml_traverse(struct VMGlobals* g, const YAML::Node & node, PyrObject *parent, PyrSlot *slot) {
	...
	switch (type)
	{
		...
		case YAML::NodeType::Sequence:
			result = newPyrArray(g->gc, (int)node.size(), 0, true);
			result->size = 0;
			SetObject(slot, result);
			if(parent) g->gc->GCWrite(parent, result);
		...
		case YAML::NodeType::Map:
		{
			...
			PyrObject *array = newPyrArray(g->gc, (int)(node.size()*2), 0, true);
			array->size = 0;
			result->size = 2; // ?
			SetObject(result->slots, array);      // array
			SetInt(result->slots+1, (int)(node.size())); // size
