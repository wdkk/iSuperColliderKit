...
int prAsCompileString(struct VMGlobals *g, int numArgsPushed)
{
	...
	a = g->sp;
	if (IsSym(a)) {
		int len = (int)strlen(slotRawSymbol(a)->name) + 1;
		if (len < 255) {
			char str[256];
...
int blockValueArray(struct VMGlobals *g, int numArgsPushed)
{
	...
	if (IsObj(b)) {
		if (slotRawObject(b)->classptr == class_array) {
			...
			int stackDepth = (int)(g->sp - stack->slots + 1);
			int stackSize = (int)ARRAYMAXINDEXSIZE(stack);
...
{
	PyrSlot *a = g->sp;
	SetInt(a, (int)g->allocPool->TotalFree());
	return errNone;
}
...
{
	PyrSlot *a = g->sp;
	SetInt(a, (int)g->allocPool->LargestFreeChunk());
	return errNone;
}
...
int prDeepFreeze(struct VMGlobals *g, int numArgsPushed)
{
	...
	int err = (int)freezer.doDeepFreeze(a);
	return err;
}
...
int prDeepCopy(struct VMGlobals *g, int numArgsPushed)
{
	...
	int err = (int)copier.doDeepCopy(a);
	return err;
}
...
int prObjectRespondsTo(struct VMGlobals *g, int numArgsPushed)
{
	...
	if (IsSym(b)) {
		...
		index = (int)(slotRawInt(&classobj->classIndex) + selector->u.index);
...
PyrMethod* GetFunctionCompileContext(VMGlobals* g)
{
	...
	int index = (int)(slotRawInt(&classobj->classIndex) + contextsym->u.index);
	meth = gRowTable[index];
...
int prUGenCodeString(struct VMGlobals *g, int numArgsPushed)
{
	...
	int ugenPrefixSize = (int)strlen(ugenPrefix);
...
void switchToThread(VMGlobals *g, PyrThread *newthread, int oldstate, int *numArgsPushed)
{
	...
	if (oldstate == tDone) {
		...
		SetObject(&oldthread->stack, gc->Stack());
		gc->ToWhite(gc->Stack());
		gc->Stack()->size = (int)(g->sp - gc->Stack()->slots + 1);
...
static int prLanguageConfig_getLibraryPaths(struct VMGlobals * g, int numArgsPushed, int pathType)
{
	...
	PyrObject * resultArray = newPyrArray(g->gc, (int)numberOfPaths, 0, true);
	SetObject(result, resultArray);
	resultArray->size = (int)numberOfPaths;
