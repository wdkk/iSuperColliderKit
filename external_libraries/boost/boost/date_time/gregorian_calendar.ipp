...
namespace date_time {
  ...
  */
  ...
  gregorian_calendar_base<ymd_type_,date_int_type_>::week_number(const ymd_type& ymd) {
    ...
    if ((week >= 1) && (week <= 52)) {
      return (int)week;  // kengo:
    }

    if (week == 53) {
      if((day==6) ||(day == 5 && is_leap_year(ymd.year))) {
        return (int)week; //under these circumstances week == 53.     // kengo:
      } else {
    ...
    return (int)week;    // kengo: //not reachable -- well except if day == 5 and is_leap_year != true
