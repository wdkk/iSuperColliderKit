...
static __inline__ void MAPPING_SEARCH(size_t * _r, int *_fl, int *_sl)
{
    ...
    if (*_r < SMALL_BLOCK) {
        *_fl = 0;
        *_sl = (int)(*_r / (SMALL_BLOCK / MAX_SLI));
    } else {
        _t = (1 << (ms_bit((int)*_r) - MAX_LOG2_SLI)) - 1;
        *_r = *_r + _t;
        *_fl = ms_bit((int)*_r);
        *_sl = ((int)*_r >> (*_fl - MAX_LOG2_SLI)) - MAX_SLI;
        *_fl -= FLI_OFFSET;
...
/******************************************************************/
    ...
    cpsize = ((b->size & BLOCK_SIZE) > new_size) ? (unsigned int)new_size : (b->size & BLOCK_SIZE);
