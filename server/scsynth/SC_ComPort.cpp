...
static void dumpOSCmsg(int inSize, char* inData)
{
	...
	if (inData[0]) {
		...
		size = (int)(inSize - (data - inData));   // kengo:
		scprintf("[ \"%s\",", addr);
	} else {
...
#else
		int size = (int)recvfrom(mSocket, mReadBuf, kMaxUDPSize , 0,      // kengo:
#endif
...
#else
		int numbytes = (int)recv(socket, msg, len - total, 0);  // kengo:
#endif
...
#else
		int numbytes = (int)sendto(socket, msg, len - total, 0, toaddr, addrlen);  // kengo:
#endif
		if (numbytes < 0) {
...
#else
		int numbytes = (int)send(socket, msg, len - total, 0);  // kengo:
#endif
		if (numbytes < 0) return total;
		total += numbytes;
		msg = (void*)((char*)msg + numbytes);
