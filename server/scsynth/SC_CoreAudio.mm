...
int PerformOSCMessage(World *inWorld, int inSize, char *inData, ReplyAddress *inReply)
{
	...
	if (inData[0] == 0) {
		...
		cmdNameLen = (int)OSCstrlen(inData);      // kengo:
...
#ifdef SC_IPHONE
    SCEX_CoreAudioAccessorCallCallback( ((float *)ioData->mBuffers[0].mData), inNumberFrames, (uint64)oscTime );
#endif
...
bool SC_iCoreAudioDriver::DriverSetup(int* outNumSamplesPerCallback, double* outSampleRate)
{
	...
	*outSampleRate = (int)sample_rate;
	...
	buflist->mBuffers[0].mDataByteSize = (int)bufferSizeBytes;  // kengo:
	...
	floatInputList->mBuffers[0].mDataByteSize = (int)bufferSizeBytes;  // kengo:
