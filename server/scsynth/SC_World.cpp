...
bool nextOSCPacket(FILE *file, OSC_Packet *packet, int64& outTime)
{
	...
	size_t read = (int)fread(packet->mData, 1, msglen, file);
	if (read != msglen)
...
SC_DLLEXPORT_C void World_NonRealTimeSynthesis(struct World *world, WorldOptions *inOptions)
{
	...
	for (; run;) {
		...
		if (world->hw->mNRTInputFile) {
			int framesRead = (int)sf_readf_float(world->hw->mNRTInputFile, inputFileBuf, fileBufFrames);  // kengo:
...
SCErr bufAlloc(SndBuf* buf, int numChannels, int numFrames, double sampleRate)
{
	...
	buf->samples  = (int)numSamples;  // kengo:
	buf->mask     = BUFMASK((int)numSamples); // for delay lines  // kengo:
