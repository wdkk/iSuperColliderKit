			...
			if(numControls > 0) {
				...
				for(i=0; i < numControls; i++){
					...
					if((childGraph->mMapControls[i]) != ptr){
						...
						bus = (int)((childGraph->mMapControls[i]) - (child->mWorld->mAudioBus));  // kengo:
						bus = (int)((float)bus / child->mWorld->mBufLength);
						scprintf("a%d", bus);
					    } else {
						bus = (int)((childGraph->mMapControls[i]) - (child->mWorld->mControlBus));  // kengo:
						scprintf("c%d", bus);
					    }
						//scprintf("bus: %d\n", bus);
					} else {
	...
	while (child) {
		...
		if (child->mIsGroup) {
			...
			for(i=0; i < numControls; i++){
				...
				if((childGraph->mMapControls[i]) != ptr){
					// it's mapped
					int bus = (int)((childGraph->mMapControls[i]) - (child->mWorld->mControlBus));    // kengo:
					//scprintf("bus: %d\n", bus);
