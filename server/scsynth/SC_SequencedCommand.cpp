...
char* allocAndRestrictPath(World *mWorld, const char* inPath, const char* restrictBase){
	...
	if(strncmp(inPath, restrictBase, strlen(restrictBase)) != 0){
		...
		offset = (int)strlen(restrictBase);
		remain -= offset;
...
void SyncCmd::Stage4()
{
	...
	SendReply(&mReplyAddress, packet.data(), (int)packet.size());
}
...
bool BufAllocReadCmd::Stage2()
{
	...
	else if (mFileOffset > fileinfo.frames) mFileOffset = (int)(fileinfo.frames);   // kengo:
	if (mNumFrames <= 0 || mNumFrames + mFileOffset > fileinfo.frames) mNumFrames = (int)(fileinfo.frames - mFileOffset);  // kengo:
...
bool AudioStatusCmd::Stage2()
{
	...
	SendReply(&mReplyAddress, packet.data(), (int)packet.size() /*kengo:*/);
