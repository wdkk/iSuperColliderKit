...
void Pitch_Ctor(Pitch *unit)
{
	...
	unit->m_minperiod = (int)(long)(unit->m_srate / unit->m_maxfreq);  // kengo:
	unit->m_maxperiod = (int)(long)(unit->m_srate / unit->m_minfreq);  // kengo:
			...
			if (ampok) {
				...
				for (i = 1; i <= maxperiod; i += binstep) {
					...
					if (octave <= maxlog2bins) {
						...
						binstep = (int)(1L << (octave - maxlog2bins));    // kengo:
					}
				}
				int startperiod = i;
	...
	while (remain) {
		...
		if (initializing) {
			...
			if (irdphase < 0) {
				if ((dlywr - dlyrd) > nsmps) {
					...
					if ((nsmps & (nova::vec<float>::size - 1)) == 0) {
						nova::copyvec_nn_simd(dlywr + ZOFF, in + ZOFF, (unsigned int)nsmps);  // kengo:
						nova::zerovec_na_simd(out + ZOFF, (unsigned int)nsmps);  // kengo:
					} else
#endif
					{
						ZCopy((int)nsmps, dlywr, in);  // kengo:
						ZClear((int)nsmps, out);  // kengo:
					}
					out += nsmps;
					in += nsmps;
					dlyrd += nsmps;
			...
			if (std::abs((float)(dlyrd - dlywr)) > nsmps) {
				...
				if ((nsmps & 15) == 0) {
					nova::copyvec_nn_simd(dlywr + ZOFF, in + ZOFF, (unsigned int)nsmps);   // kengo:
					nova::copyvec_nn_simd(out + ZOFF, dlyrd + ZOFF, (unsigned int)nsmps);  // kengo:
				} else
#endif
				{
					ZCopy((int)nsmps, dlywr, in);
					ZCopy((int)nsmps, out, dlyrd);
				}
				out += nsmps;
				in += nsmps;
				dlyrd += nsmps;
...
static bool DelayUnit_AllocDelayLine(DelayUnit *unit, const char * className)
{
	long delaybufsize = (long)ceil(unit->m_maxdelaytime * SAMPLERATE + 1.f);
	delaybufsize = delaybufsize + BUFLENGTH;
	delaybufsize = NEXTPOWEROFTWO((int)delaybufsize);    // kengo: // round up to next power of two
	unit->m_fdelaylen = unit->m_idelaylen = delaybufsize;
...
void DelayN_next(DelayN *unit, int inNumSamples)
{
	...
	if (delaytime == unit->m_delaytime) {
		DelayN_delay_loop<false>(out, in, iwrphase, dsamp, mask, dlybuf, inNumSamples, (int)unit->m_idelaylen);  // kengo:
	} else {
		float next_dsamp = CalcDelay(unit, delaytime);
		float dsamp_slope = CALCSLOPE(next_dsamp, dsamp);
...
void DelayN_next_z(DelayN *unit, int inNumSamples)
{
	...
	if (delaytime == unit->m_delaytime) {
		DelayN_delay_loop<true>(out, in, iwrphase, dsamp, mask, dlybuf, inNumSamples, (int)unit->m_idelaylen);  // kengo:
	} else {
		float next_dsamp = CalcDelay(unit, delaytime);
		float dsamp_slope = CALCSLOPE(next_dsamp, dsamp);
...
void GrainTap_next(GrainTap *unit, int inNumSamples)
{
	...
	bufsize = (int)unit->bufsize;  // kengo:
