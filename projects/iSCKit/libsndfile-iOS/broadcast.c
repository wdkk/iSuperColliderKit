...
{	PSF_BROADCAST_VAR * data ;
	...
	if ((data = calloc (1, datasize)) != NULL)
		data->size = (int)datasize; // kengo:cast
...
{	char added_history [256] ;
	...
	if (psf->broadcast_var == NULL)
	{	int size = (int)(datasize + added_history_len + 512);   // kengo:cast
	...
	len = (int)strlen(psf->broadcast_var->binfo.coding_history);    // kengo:cast

	if (len > 0 && psf->broadcast_var->binfo.coding_history [len] != '\n')
		strncat (psf->broadcast_var->binfo.coding_history, "\r\n", 2) ;

	if (psf->file.mode == SFM_WRITE)
		strncat (psf->broadcast_var->binfo.coding_history, added_history, (int)strlen (added_history)) ;

	psf->broadcast_var->binfo.coding_history_size = (int)strlen(psf->broadcast_var->binfo.coding_history); // kengo:cast
...
{	char chnstr [16] ;
	...
	*/
	...
	count = snprintf (added_history, added_history_max,
							...
							psfinfo->samplerate, (int)width, chnstr, PACKAGE, VERSION); // kengo:cast
