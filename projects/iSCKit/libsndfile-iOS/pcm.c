...
{	int		total ;
	...
	if (CPU_IS_LITTLE_ENDIAN)
		endswap_short_array (ptr, (int)len);   // kengo:
...
{	int		total ;
	...
	total = (int)psf_fread (ptr, sizeof (int), len, psf) ;
	if (CPU_IS_LITTLE_ENDIAN)
...
{	unsigned char	*ucptr ;
	...
	{	ucptr -= 2 ;
		...
		value = (int)lrintf (scaled_value);    // kengo:
		ucptr [1] = value >> 16 ;
		ucptr [0] = value >> 24 ;
		} ;
} /* f2bes_clip_array */
...
{	unsigned char	*ucptr ;
	...
	while (--count >= 0)
	{	ucptr -= 2 ;
		value = (int)lrintf (src [count] * normfact) ;  // kengo:
		ucptr [0] = value ;
		ucptr [1] = value >> 8 ;
		} ;
} /* f2les_array */
...
{	unsigned char	*ucptr ;
	...
	{	ucptr -= 3 ;
		...
		value = (int)lrint (scaled_value) ;    // kengo:
		ucptr [0] = value >> 24 ;
		ucptr [1] = value >> 16 ;
		ucptr [2] = value >> 8 ;
		} ;
...
{	unsigned char	*ucptr ;
	...
	while (--count >= 0)
	{	ucptr -= 3 ;
		value = (int)lrint (src [count] * normfact) ;    // kengo:
		ucptr [0] = value ;
		ucptr [1] = value >> 8 ;
		ucptr [2] = value >> 16 ;
		} ;
