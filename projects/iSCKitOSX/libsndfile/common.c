...
{	va_list	argptr ;
	...
	maxlen = (int)strlen ((char*) psf->header) ;
		...
		{	case ' ' : /* Do nothing. Just used to space out format string. */
			...
			case '8' :
					...
					{	longdata = countdata & 0xFFFFFFFF ;
						header_put_be_int (psf, (int)longdata); // kengo:cast
						count += 4 ;
						}
					else if (psf->rwf_endian == SF_ENDIAN_LITTLE && trunc_8to4 == SF_TRUE)
					{	longdata = countdata & 0xFFFFFFFF ;
						header_put_le_int (psf, (int)longdata); // kengo:cast
						count += 4 ;
						}
					break ;
			...
			case 's' :
					...
					size = (int)strlen (strptr) + 1 ;
					size += (size & 1) ;
					if (psf->rwf_endian == SF_ENDIAN_BIG)
						header_put_be_int (psf, (int)size); // kengo:cast
					else
						header_put_le_int (psf, (int)size); // kengo:cast
...
header_seek (SF_PRIVATE *psf, sf_count_t position, int whence)
{
	...
	{	case SEEK_SET :
			...
			if (position > psf->headend)
				psf->headend += (int)psf_fread (psf->header + psf->headend, 1, position - psf->headend, psf);   // kengo:cast(all)
			psf->headindex = (int)position ;    // kengo:cast
			break ;
...
{	va_list			argptr ;
	...
	if (! format)
		return (int)psf_ftell (psf) ;   // kengo:cast
...
{	static int32_t value = -1 ;
	...
	if (value == -1)
	{
		...
		value = (int)(tv.tv_sec + tv.tv_usec);  // kengo:cast
#else
		value = time (NULL) ;
#endif
		} ;
