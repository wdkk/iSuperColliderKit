...
{	int		exponent, mantissa ;
	...
	in *= 0x20000000 ;
	mantissa = (int)lrint (floor (in)) ;    // kengo:
...
{	while (--count >= 0)
	{	dest [count] = (int)lrint (scale * src [count]) ;   // kengo:
		} ;
} /* d2i_array */
...
{	while (--count >= 0)
	{	float tmp = scale * src [count] ;
		...
		if (CPU_CLIPS_POSITIVE == 0 && tmp > (1.0 * INT_MAX))
			...
			dest [count] = (int)lrint (tmp) ;   // kengo:
		} ;
} /* d2i_clip_array */
...
{	int			bufferlen ;
	...
	readcount = (int)psf_fread (ptr, sizeof (double), len, psf) ;
	...
	{	endswap_double_array (ptr, (int)readcount) ;   // kengo:
		return readcount ;
		} ;
...
{	int			bufferlen, writecount ;
	...
	if (psf->peak_info)
		double64_peak_update (psf, ptr, (int)len, 0) ;   // kengo:
