...
{	static const char	*filename = "MATLAB 5.0 MAT-file, written by " PACKAGE "-" VERSION ", " ;
	...
	psf_binheader_writef (psf, "bb", filename, (int)strlen (filename), psf->u.cbuf, (int)strlen (psf->u.cbuf) + 1) ;
	...
	psf_binheader_writef (psf, "44b", MAT5_TYPE_SCHAR, (int)strlen (sr_name), sr_name, make_size_t (16)) ;
	...
	psf_binheader_writef (psf, "44b", MAT5_TYPE_SCHAR, (int)strlen (wd_name), wd_name, (int)strlen (wd_name)) ;
...
{	char	name [32] ;
	...
	{	case MAT5_TYPE_DOUBLE :
				{	double	samplerate ;
					...
					psf->sf.samplerate = (int)(lrint (samplerate));   // kengo:
					} ;
				break ;
