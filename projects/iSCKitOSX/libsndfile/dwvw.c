...
{	DWVW_PRIVATE *pdwvw ;
	...
	while (len > 0)
	{	readcount = (len >= bufferlen) ? bufferlen : (int)len;  // kengo:cast
...
{	DWVW_PRIVATE *pdwvw ;
	...
	bufferlen = ARRAY_LEN (psf->u.ibuf) ;
	while (len > 0)
	{	writecount = (len >= bufferlen) ? bufferlen : (int)len; // kengo:cast
...
{	DWVW_PRIVATE *pdwvw ;
	...
	{	writecount = (len >= bufferlen) ? bufferlen : (int)len; // kengo:cast
		for (k = 0 ; k < writecount ; k++)
			iptr [k] = (int)lrintf (normfact * ptr [total + k]) ;   // kengo:cast
		count = dwvw_encode_data (psf, pdwvw, iptr, writecount) ;
...
{	DWVW_PRIVATE *pdwvw ;
	...
	{	writecount = (len >= bufferlen) ? bufferlen : (int)len ;    // kengo:cast
		for (k = 0 ; k < writecount ; k++)
			iptr [k] = (int)lrint (normfact * ptr [total + k]) ;
		count = dwvw_encode_data (psf, pdwvw, iptr, writecount) ;
