...
{	SD2_RSRC rsrc ;
	...
	{	if (str_rsrc [k].value_len == 0)
		...
		str_rsrc [k].name [0] = (int)strlen (str_rsrc [k].name) - 1 ;
		} ;
	...
	*/
	...
	write_char (rsrc.rsrc_data, 0x30, (int)strlen (psf->file.name.c)) ;
...
{	SD2_RSRC rsrc ;
	...
	rsrc.rsrc_len = (int)psf_get_filelen (psf) ;    // kengo:cast
...
{	char name [32], value [32] ;
	...
	{	int slen ;
		...
		if (rsrc_id == 1000 && rsrc->sample_size == 0)
			rsrc->sample_size = (int)strtol (value, NULL, 10) ;
		else if (rsrc_id == 1001 && rsrc->sample_rate == 0)
			rsrc->sample_rate = (int)strtol (value, NULL, 10) ;
		else if (rsrc_id == 1002 && rsrc->channels == 0)
			rsrc->channels = (int)strtol (value, NULL, 10) ;
		} ;
