...
{
	char * chunk = (char*) alloc.alloc(sizeof(scfft) + scfft_trbufsize((int)fullsize));  // kengo:
	if (!chunk)
		return NULL;
	...
	f->nfull = (unsigned int)fullsize;  // kengo:
	f->nwin  =  (unsigned int)winsize;  // kengo:
	f->log2nfull = LOG2CEIL((int)fullsize);  // kengo:
	f->log2nwin  = LOG2CEIL((int)winsize);  // kengo:
	f->wintype = wintype;
	f->indata  = indata;
	f->outdata = outdata;
	f->trbuf   = trbuf;
	...
	memset(trbuf, 0, scfft_trbufsize((int)fullsize));     // kengo:
