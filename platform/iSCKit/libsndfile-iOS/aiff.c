	...
	*/
	...
	{	psf_binheader_readf (psf, "m", &marker) ;
		...
		{	case FORM_MARKER :
					...
					{	dword = (int)(psf->filelength - 2 * sizeof (dword));    // kengo:cast
			...
			default :
					...
					if ((dword = (int)(psf_ftell (psf)) ) & 0x03)   // kengo:cast
...
{	int error = 0, bytesread, subformat ;
	...
	*/
	...
	{	unsigned char encoding_len ;
		...
		comm_fmt->size = SF_MIN (sizeof (psf->u.scbuf), (int)(make_size_t(comm_fmt->size)) );   // kengo:cast
	...
	*/
	...
	{	switch (paiff->chunk4.l [k].chunk)
		{	case FORM_MARKER :
			...
			case COMM_MARKER :
				psf->headindex = (int)(paiff->chunk4.l [k].offset); // kengo:cast
				comm_frames = (int)psf->sf.frames;  // kengo:cast
				comm_size = (int)paiff->chunk4.l [k].len;   // kengo:cast
...
{	sf_count_t		current ;
	...
	comm_frames = (psf->sf.frames > 0xFFFFFFFF) ? 0xFFFFFFFF : (int)psf->sf.frames; // kengo:cast
	...
	{	case SF_FORMAT_PCM_S8 :
		...
		case SF_FORMAT_IMA_ADPCM :
				...
				/* Override standard value here.*/
				bit_width = 16 ;
				comm_frames = (unsigned int)(psf->sf.frames / AIFC_IMA4_SAMPLES_PER_BLOCK); // kengo:cast
				break ;
...
{	int	k, slen ;
	...
	{	if (psf->strings [k].type == 0)
		...
		{	case SF_STR_SOFTWARE :
				slen = (int)strlen(psf->strings [k].str) ;  // kengo:cast
