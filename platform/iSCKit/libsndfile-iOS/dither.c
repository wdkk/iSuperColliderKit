...
{	DITHER_DATA *pdither ;
	...
	{	writecount = (len >= bufferlen) ? bufferlen : (int) len ;
		...
		thiswrite = (int)pdither->write_short (psf, (short*) pdither->buffer, writecount) ; // kengo:cast
		total += thiswrite ;
		len -= thiswrite ;
...
{	DITHER_DATA *pdither ;
	...
	{	writecount = (len >= bufferlen) ? bufferlen : (int) len ;
		...
		thiswrite = (int)(pdither->write_int (psf, (int*) pdither->buffer, writecount));    // kengo:cast
		total += thiswrite ;
		len -= thiswrite ;
		if (thiswrite < writecount)
			break ;
...
{	DITHER_DATA *pdither ;
	...
	{	writecount = (len >= bufferlen) ? bufferlen : (float) len ;
		...
		thiswrite = (int)(pdither->write_float (psf, (float*) pdither->buffer, writecount));    // kengo:cast
		total += thiswrite ;
		len -= thiswrite ;
		if (thiswrite < writecount)
			break ;
...
{	DITHER_DATA *pdither ;
	...
	{	writecount = (len >= bufferlen) ? bufferlen : (double) len ;
		...
		thiswrite = (int)(pdither->write_double (psf, (double*) pdither->buffer, writecount));  // kengo:cast
		total += thiswrite ;
		len -= thiswrite ;
		if (thiswrite < writecount)
			break ;
