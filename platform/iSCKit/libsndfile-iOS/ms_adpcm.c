...
{	MSADPCM_PRIVATE	*pms ;
	...
	{	pms->dataremaining = (int)psf->datalength;  // kengo:cast

		if (psf->datalength % pms->blocksize)
			pms->blocks = (int)(psf->datalength / pms->blocksize + 1); // kengo:cast
		else
			pms->blocks = (int)(psf->datalength / pms->blocksize); // kengo:cast
...
{	int	count, total = 0, indx = 0 ;
	...
	{	if (pms->blockcount >= pms->blocks && pms->samplecount >= pms->samplesperblock)
		...
		count = (int)((pms->samplesperblock - pms->samplecount) * pms->channels); // kengo:cast
...
{	MSADPCM_PRIVATE 	*pms ;
	...
	{	readcount = (len > 0x10000000) ? 0x10000000 : (int) len ;
		...
		count = (int)msadpcm_read_block (psf, pms, ptr, readcount) ; // kengo:cast
...
{	MSADPCM_PRIVATE *pms ;
	...
	{	readcount = (len >= bufferlen) ? bufferlen : (int)len ;
		count = (int)msadpcm_read_block (psf, pms, sptr, readcount) ; // kengo:cast
...
{	MSADPCM_PRIVATE *pms ;
	...
	while (len > 0)
	{	readcount = (len >= bufferlen) ? bufferlen : (int)len ;
		count = (int)msadpcm_read_block (psf, pms, sptr, readcount);   // kengo:
		for (k = 0 ; k < readcount ; k++)
...
{	MSADPCM_PRIVATE *pms ;
	...
	newblock	= (int)(offset / pms->samplesperblock);   // kengo:
...
{	int		count, total = 0, indx = 0 ;
	...
	while (indx < len)
	{	count = (int)(pms->samplesperblock - pms->samplecount) * pms->channels ;   // kengo:
...
{	MSADPCM_PRIVATE *pms ;
	...
	{	writecount = (len > 0x10000000) ? 0x10000000 : (int) len ;
		...
		count = (int)msadpcm_write_block (psf, pms, ptr, writecount) ;   // kengo:
