...
{	PAF24_PRIVATE	*ppaf24 ;
	...
	*/
	...
	{	if (psf->file.mode == SFM_READ)
		...
		ppaf24->max_blocks = (int)(psf->datalength / ppaf24->blocksize + 1);   // kengo:
		}
	else
		ppaf24->max_blocks = (int)(psf->datalength / ppaf24->blocksize);   // kengo:
...
{	PAF24_PRIVATE	*ppaf24 ;
	...
	newblock	= (int)(offset / ppaf24->samplesperblock);   // kengo:
...
{	PAF24_PRIVATE *ppaf24 ;
	...
	total = paf24_read (psf, ppaf24, ptr, (int)len);       // kengo:
...
{	PAF24_PRIVATE 	*ppaf24 ;
	...
	{	writecount = (len >= bufferlen) ? bufferlen : (int)len ;
		for (k = 0 ; k < writecount ; k++)
			iptr [k] = (int)lrintf (normfact * ptr [total + k]) ;   // kengo:
		count = paf24_write (psf, ppaf24, iptr, writecount) ;
		total += count ;
		len -= writecount ;
		if (count != writecount)
...
{	PAF24_PRIVATE 	*ppaf24 ;
	...
	{	writecount = (len >= bufferlen) ? bufferlen : (int)len ;
		for (k = 0 ; k < writecount ; k++)
			iptr [k] = (int)lrint (normfact * ptr [total+k]) ;   // kengo:
		count = paf24_write (psf, ppaf24, iptr, writecount) ;
		total += count ;
		len -= writecount ;
		if (count != writecount)
