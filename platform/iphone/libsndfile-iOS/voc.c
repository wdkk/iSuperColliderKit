...
{	VOC_DATA	*pvoc ;
	...
	{	unsigned char bitwidth, channels ;
		...
		{	int temp_size = (int)(psf->filelength - 31); // kengo:
		...
		{	/* Hack for reading files produced using
			...
			*/
			...
			size = (int)(psf->filelength - 30); // kengo:
			psf->dataend = 0 ;
			}
		else if (size + 31 > psf->filelength)
		{	psf_log_printf (psf, "Seems to be a truncated file.\n") ;
			size = (int)(psf->filelength - 31); // kengo:
			}
		else if (size + 31 < psf->filelength)
...
{	sf_count_t	current ;
	...
	*/
	...
	{	int length ;
		...
		{	case SF_FORMAT_PCM_U8 :
					psf->bytewidth = 1 ;
					length = (int)(psf->sf.frames * psf->sf.channels * psf->bytewidth + 12);   // kengo:
