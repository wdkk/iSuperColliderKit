...
{	unsigned char	channel, bitwidth, loop_type, byte ;
	...
	{	psf_log_printf (psf, " Datalength     : %d (truncated data??? %d)\n", data_length, psf->filelength - psf->dataoffset) ;
		data_length = (int)(psf->filelength - psf->dataoffset); // kengo:
		}
	else
...
{	SDS_PRIVATE *psds ;
	...
	total = sds_read (psf, psds, ptr, (int)len);     // kengo:
...
{	SDS_PRIVATE	*psds ;
	...
	newblock = (int)(seek_from_start / psds->samplesperblock); // kengo:
...
{	SDS_PRIVATE *psds ;
	...
	total = sds_write (psf, psds, ptr, (int)len); // kengo:
