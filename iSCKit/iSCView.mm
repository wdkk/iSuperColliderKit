...
void SCView::touchDownAction(SCPoint where, UITouch *touch)
{
	...
	int clickCount = (int)[touch tapCount];
	//int buttonNum = 0;    // kengo:
    if (mObj) {
        VMGlobals *g = gMainVMGlobals;
        g->canCallOS = true;
	...
	if (strcmp(name, "bounds")==0) {
		...
		err = slotGetSCRect(slot, &screct);
				...
				if(mTop->isScroller()) {
					...
					setBounds(screct);
					//[(iSCGraphView*)mTop->GetUIView() setFrameSizeToMinimum];  // kengo:comment out
...
*/
	...
	if (strcmp(name, "dragLabel")==0) {
		if(isKindOfSlot(slot, class_string)) {
			...
			if(mDragLabel) { mDragLabel = nil; } // kengo:
			mDragLabel = [[NSString alloc] initWithUTF8String:pstring->s /*length: pstring->size*/];    // kengo:
			return errNone;
		} else if (IsNil(slot)) {
			if(mDragLabel) { mDragLabel = nil; } // kengo:
			mDragLabel = 0;
...
void SCView::beginDrag(SCPoint where)
{
	...
	if (mObj) {
		...
		if (isKindOfSlot(&stringSlot, class_string)) {
			string = [NSString stringWithUTF8String: slotRawString(&stringSlot)->s /*length: slotRawString(&stringSlot)->size*/];
		}
...
int allocSlotStrVal(PyrSlot *slot, char **str)
{
	...
	if (IsSym(slot)) {
			len = (int)strlen(slotRawSymbol(slot)->name);
			*str = new char[len+1];
